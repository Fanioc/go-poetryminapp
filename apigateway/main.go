...
func main() {
  ...
  // method: sum, concat, uppercase, and count.
  {
    ...
    var (
      ...
      instancer, err = etcdv3.NewInstancer(client, "/book/list/", logger)
    )
    if err != nil {
      panic("cannot find discovered server:" + "/book/list/")
    }
    ...
    r.HandleFunc("/book/list/", func(writer http.ResponseWriter, request *http.Request) {
      bookList, err := bookService.GetBookList(request.Context(), book.BookListParams{Page: 1, Limit: 10})
      if err != nil {
        fmt.Println(err)
        writer.Write([]byte("xxx"))
      }
      bl := bookList.(*book.BookList)
...
func bookFactory(makeBookEndpoint func(bookClient book.BookServiceClient) endpoint.Endpoint) sd.Factory {
...
func makeBookListEndpoint(bookClient book.BookServiceClient) endpoint.Endpoint {
